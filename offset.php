    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            function setMarker(map, addressDetails) {
                lat = new google.maps.LatLng(addressDetails[1], addressDetails[2]);
                ...
                if (markers.length != 0) {
                    for (i = 0; i < markers.length; i++) {
                        ...
                        if (lat.equals(pos)) {
                            var newLat = lat.lat() + (Math.random() - 4) / 100;
                            var newLng = lat.lng() + (Math.random() - 4) / 100;
                            lat = new google.maps.LatLng(newLat, newLng);
                        }
                    }
                }
